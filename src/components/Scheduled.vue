...
const fetchScheduledReports = async() => {
  ...
  error.value = null
  try {
    const response = await api.getScheduledReports()
...
<template>
  <v-container fluid class="fill-height py-4">
    ...
    <v-row class="mb-2">
      <v-col cols="12">
        <div class="d-flex align-center gap-2">
          ...
          <span class="text-h6 font-weight-bold">Geplande rapportages</span>
        </div>
      </v-col>
    </v-row>
    <!-- LOAD -->
    ...
    <v-row v-else-if="reports.length === 0">
      <v-col cols="12">
        <v-alert type="info" variant="tonal" text="Geen geplande rapportages gevonden." />
      </v-col>
    </v-row>
    <!-- List with Result -->
    <v-row v-else>
